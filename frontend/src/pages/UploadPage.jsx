...
import { acknowledgeDisclosure, getDisclosureStatus } from "../api/user.js";
import AIDisclosureModal from "../components/AIDisclosureModal.jsx";
...
export default function UploadPage() {
  ...
  // null = checking; false = show disclosure modal; true = upload form unlocked
  const [disclosureAcknowledged, setDisclosureAcknowledged] = useState(null);

  useEffect(() => {
    getDisclosureStatus()
      .then((data) => setDisclosureAcknowledged(Boolean(data.ai_disclosure_acknowledged)))
      .catch(() => setDisclosureAcknowledged(true)); // don't block uploads if the check fails
  }, []);

  async function handleAcknowledgeDisclosure(dontShowAgain) {
    if (dontShowAgain) {
      try {
        await acknowledgeDisclosure();
      } catch {
        // If persisting fails, the modal simply reappears next visit.
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {disclosureAcknowledged === false && (
        <AIDisclosureModal onAcknowledge={handleAcknowledgeDisclosure} />
      )}
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {disclosureAcknowledged ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center text-gray-400 text-sm">
            {disclosureAcknowledged === false
              ? "Please review the AI disclosure notice to continue."
              : "Loading..."}
          </div>
        )}
      </main>
